...
class WallSegmentation:
    ...
    def segment_wall(self, image_path):
        print(f"[*] Segmenting wall with U²-Net on: {image_path}")
        ...
        pred = d0[:, 0, :, :]
        pred = F.interpolate(pred.unsqueeze(0), size=image.size[::-1], mode='bilinear').squeeze().cpu().numpy()
        pred = (pred - pred.min()) / (pred.max() - pred.min() + 1e-8)
        mask = (pred * 255).astype(np.uint8)
        mask = cv2.threshold(mask, 128, 255, cv2.THRESH_BINARY)[1]

        print("[*] Wall segmentation completed with U²-Net!")
        return mask
